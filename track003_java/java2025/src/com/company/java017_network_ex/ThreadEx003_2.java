...
class Count32  implements Runnable { //1) extends Thread  , implements Runnable
	@Override public void run() { //2) todo
		for(int i=10;i>0; i--) {
			////1. 카운트
			if(Thread.currentThread().isInterrupted()) {  //###  stop2)  반복안에서 10~1
			...
			/////2. 1초마다 쉬었다가 들어가기
			try { Thread.sleep(1000); }		 //###  stop3)              true
...
public class ThreadEx003_2 {
	public static void main(String[] args) { // 실행클래스-main
		...
		Thread timer1 = new Thread(new Count32());  timer1.start(); //3)
