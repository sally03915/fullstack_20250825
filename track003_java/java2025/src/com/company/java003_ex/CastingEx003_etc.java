...
public class CastingEx003_etc {
	...
	public static void main(String[] args) {
        // 변수 선언
        int steps, workoutMinutes, sleepHours, totalScore, healthLevel;
        double avgScore;
        Scanner scanner = new Scanner(System.in);

        // 입력
        System.out.print("하루 걸음 수 입력 > "); steps = scanner.nextInt();
        System.out.print("운동 시간(분) 입력 > "); workoutMinutes = scanner.nextInt();
        System.out.print("수면 시간(시간) 입력 > "); sleepHours = scanner.nextInt();

        // 처리
        // 각 항목을 점수화 (예시 기준)
        int stepScore = steps / 1000;           // 1000걸음당 1점
        int workoutScore = workoutMinutes / 10; // 10분당 1점
        int sleepScore = sleepHours * 2;        // 1시간당 2점

        totalScore = stepScore + workoutScore + sleepScore;
        avgScore = totalScore / 3.0;
        healthLevel = (int)avgScore / 2;        // 평균 점수를 기준으로 레벨 산정
        ...
        System.out.printf("%d\t%d\t\t%d\t\t%d\t%.2f\t%d\n", 
            steps, workoutMinutes, sleepHours, totalScore, avgScore, healthLevel);
	}
